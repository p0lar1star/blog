...
脚本原理参考：利用符号执行去除控制流平坦化https://security.tencent.com/index.php/blog/msg/112

原文给出的脚本已经不能使用，大佬维护的脚本：https://github.com/cq674350529/deflat

在Ubuntu18.04LTS，python3，angr版本指定为8.19.4.5的环境下，可以运行

命令可类比：python3 deflat.py -f check_passwd_x8664_flat --addr 0x400530

另有一份脚本deflat1.py（见下）

命令可类比：python3 deflat1.py check_passws_x8664_flat 0x400530（其实就是不用加 -f 和 --addr）
...
建议到此为止，后面解题过程过于脑洞，大致是输入函数名来控制迷宫的上下左右，不值得花时间